...
std::string LcNet_FormatMessage(DWORD hr)
{
	LPVOID	pBuf;
	FormatMessage( 
...
std::string LcNet_FormatMessage()
{
	LPVOID	pBuf;
...
std::string LcNet_GetNetworkError(DWORD hr)
{
	return LcNet_FormatMessage(hr);
}
...
INT LcNet_WSAGetError()
{
	...
	auto err = LcNet_FormatMessage(hr);
	return -1;
}
...
void LcNet_SocketAddr(SOCKADDR_IN* pOut, const std::string& sIp, const std::string& sPort)
{
	auto str_ip = sIp.c_str();
	auto str_pt = sPort.c_str();
	memset(pOut, 0, sizeof(SOCKADDR_IN));

	pOut->sin_family=AF_INET;
	pOut->sin_port=htons(atoi(str_pt));
	if(strlen(str_ip)>1)
		pOut->sin_addr.s_addr = inet_addr(str_ip);
	else
