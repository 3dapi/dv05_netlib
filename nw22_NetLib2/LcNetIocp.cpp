...
void CLcNetIocp::Destroy()
{
	...
	for(i=0; i<m_nCPU; ++i)
	{
		PostQueuedCompletionStatus(m_hIocp, 0, 0U, nullptr);
		Sleep(10);
	}
...
INT CLcNetIocp::Connect(const char* sIp, const char* sPort)
{
	if(sIp  ){	m_sIp = sIp;	}
	if(sPort){	m_sPt = sPort;	}
	...
	if(SUCCEEDED(LcNet_SocketConnect(m_scH, &m_sdH)))
	{
		...
		m_hIocp = LcNet_IocpPortCreate((SOCKET)INVALID_HANDLE_VALUE, nullptr, 0);
		...
		if(nullptr == LcNet_IocpPortCreate(m_pIoH.scH, m_hIocp, &m_pIoH))
			return -1;
	...
	*iRcv = 0;
	...
	else
	{
		...
		*iRcv = 0;
		...
		const auto& pHst = LcNet_Find_Socket(m_vIoH.begin(), m_vIoH.end(), scT);

		if(pHst == m_vIoH.end() || !*pHst)
		{
			return -1;
		}
...
{
	LcNetIocpHost*	pIoH	= nullptr;								// 완료 통지 주소
	TlnOVERLAP*		pOL		= nullptr;								// Overlapped주소 Send: olSnd, Recv: olRcv
