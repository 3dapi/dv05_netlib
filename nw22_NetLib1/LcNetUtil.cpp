...
std::string LcNet_ErrorFormatMessage()
{
	LPVOID	pBuf;
	...
	std::string ret = (const char*)pBuf;
	LocalFree( pBuf );
	return ret;
}
...
INT LcNet_WSAGetError()
{
	...
	std::string err = LcNet_ErrorFormatMessage();
	return hr;
}
...
INT LcNet_WSAStartup()
{
	...
	hr = WSAEnumProtocolsA(nullptr, nullptr, &iBuf);
...
INT LcNet_SocketErrorCheck(INT hr)
{
	...
	{
		hr = WSAGetLastError();
		bErr = (WSA_IO_PENDING ==hr || WSAEWOULDBLOCK==hr || WSAEISCONN==hr) ? 1 : 0;

		if(0 == bErr)
		{
			std::string err = LcNet_ErrorFormatMessage(hr);
			printf("Err:%s\n", err.c_str());
			return -1;
		}
	}
...
INT LcNet_SocketTcpCreate(SOCKET* pScH, BOOL bOverLapped)
{
	...
	if(bOverLapped)
		*pScH = WSASocket(AF_INET, SOCK_STREAM, 0, nullptr, 0, WSA_FLAG_OVERLAPPED);
	else
...
//	scCln = WSAAccept(scH, (SOCKADDR*)&sdCln, &iSizeAdd, nullptr, nullptr);
...
{
	return (HANDLE)_beginthreadex(nullptr, 0, (LPBEGIN_THREAD_EX)pFunc, pParam, dFlag, (unsigned*)dId);
}
...
{
	return CreateEvent(nullptr, bManualReset, bIntitialState, nullptr);
}
...
LcNet_LocalIpAddress()
{
	...
	//설마 네트워크 아이피를 128개 이상 가진 녀석은 없을 것!!!!!
	std::vector<std::string> ipList;
	for(i=0;pHost->h_addr_list[i] && i<128; ++i)
	{
		memcpy(&sdH.sin_addr, pHost->h_addr_list[i], pHost->h_length);
		const char* ip = inet_ntoa(sdH.sin_addr);
		ipList.push_back(ip);
		++nIp;
	}
...
INT LcNet_PacketEncode(void* pOut					// Output Packet
					  ...
					  , INT iSndM					// Packet contents length
						...
						)
{
	...
	memcpy(sOut + 0	   , &iTot, iPstL);				// 패킷 전체길이 복사
	memcpy(sOut + iPstL, &nMsg, iPstM);				// 패킷 종류 복사
	memcpy(sOut + iHead,  sMsg, iSndM);				// 패킷 메시지 복사.
	memcpy(sOut + iLen ,  sEnc, iPstT);				// 암호화 방식 복사
					  ...
					  )
{
	...
	memcpy(sEnc, sIn + iRcvM - iPstT, iPstM);			// 암호화 종류를 가져온다.
	...
	memcpy(&iTot, sIn + 0	, iPstL);					// 전체 길이를 가져온다.
	...
	memcpy(nMsg, sIn + iPstL	  , iPstM);				// 메시지의 종류를 가저온다.
	memcpy(sMsg, sIn + iPstL+iPstM, iLen );				// 메시지를 가져온다.
